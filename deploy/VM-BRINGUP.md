...
echo 'net.ipv4.ip_unprivileged_port_start=443' | sudo tee /etc/sysctl.d/99-nextvault-port443.conf
sudo sysctl --system
...
podman images | grep nextvault    # confirm localhost/nextvault:latest exists

# Build the egress allowlist proxy image (SSO -> Entra path, see step 4b).
deploy/scripts/build-egress-proxy.sh
podman images | grep nextvault-egress-proxy     # confirm localhost/nextvault-egress-proxy:latest exists
```
...
`nextvault-postgres` server cert SAN is fixed (`nextvault-postgres`); the Caddy cert SAN must be
your real `$FQDN`.
...
> `systemctl --user restart nextvault`.
...
which the `nextvault-egress-proxy` already restricts to `login.microsoftonline.com` and
`graph.microsoft.com`.
...
systemctl --user status nextvault nextvault-postgres nextvault-caddy --no-pager

# Tail logs if anything is not 'active (running)':
journalctl --user -u nextvault-postgres -u nextvault -u nextvault-caddy -n 100 --no-pager
```

Common first-boot snag — **Postgres TLS key permissions under rootless userns**:
if `nextvault-postgres` logs `private key file ... has group or world access`, see
...
- `journalctl --user -u nextvault-egress-proxy` shows the `CONNECT ... login.microsoftonline.com` line.
...
`systemctl --user restart nextvault`. Admins still reach `/admin` via
...
- [ ] **Pin the image digest**: replace `localhost/nextvault:latest` in the
...
systemctl --user restart nextvault-postgres.service nextvault-caddy.service
```
...
```bash
systemctl --user stop nextvault-caddy.service nextvault.service nextvault-postgres.service
podman volume rm nextvault-data nextvault-pgdata nextvault-caddy-data 2>/dev/null || true
